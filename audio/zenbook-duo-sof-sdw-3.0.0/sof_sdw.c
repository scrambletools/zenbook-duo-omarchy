...
static const struct dmi_system_id b9406caa_dmi_table[] = {
	...
	{
		/* Zenbook Duo: same ghost RT722 on link 3; upstream master
		 * carries the same DMI entry with driver_data ghost_realtek. */
		.matches = {
			DMI_MATCH(DMI_SYS_VENDOR, "ASUS"),
			DMI_EXACT_MATCH(DMI_BOARD_NAME, "UX8407AA"),
		},
	},
	{}
};
					 ...
					 struct asoc_sdw_endpoint *sof_end)
{
	...
	if (ghost)
		dev_info(dev, "ignoring unattached firmware ghost %s\n",
			 sof_end->codec_name);
